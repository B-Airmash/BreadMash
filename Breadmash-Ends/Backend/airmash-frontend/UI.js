                ...
                if (!(0 == msgRankings[i].x && 0 == msgRankings[i].y)) {
                    ...
                    if (null == minimapMobs[msgRankings[i].id]) {
                        var mobTextureName = "minimapMob";
                            ...
                            0 == i && (mobTextureName = "minimap1");
                            1 == i && (mobTextureName = "minimap2"); 
                            2 == i && (mobTextureName = "minimap3");
...
UI.addChatLine = function(msg, text, msgType) {
    if (!ignoredPlayerIdSet[msg.id]) {
       ...
       text= ProfanityFilter(textToFilter);
        ...
        if (UI.escapeHTML(text, true).toUpperCase() === "-BREAD-PING" || UI.escapeHTML(text, true).toUpperCase() === "-SWAM-PING"){ 
          let pongText = "-BREAD-PONG Breadmash Player 🍞🛩️";
          // Network.sendChat(pongText);
          Network.sendWhisper(msg.id, pongText);
        }
    }
};
